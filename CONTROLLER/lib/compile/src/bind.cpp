...
PYBIND11_MODULE(py_lib, handle) {
	...
	handle.def("new_MCU_Instruction", [](
		...
		uint16_t instrution_id;
		uint8_t* data = new_MCU_Instruction(id, action, target, max_vel, max_acc, micro_step, srd_mode, &instrution_id);
		py::bytes result = py::reinterpret_steal<py::object>(PYBIND11_BYTES_FROM_STRING_AND_SIZE((char*)data, sizeof(MCU_Instruction)));
		delete[] data;
	...
	handle.def("get_MCU_Instruction_data", [](char* instrution) {
		...
		uint16_t	instrution_id;
		uint16_t	crc;
		get_MCU_Instruction_data((uint8_t*)instrution, &target, &max_vel, &max_acc, &micro_step, &srd_mode, &action, &id, &instrution_id, &crc);
		return std::make_tuple(target, max_vel, max_acc, micro_step, srd_mode, action, id, instrution_id, crc);
	});
	...
	handle.def("new_CTRL_Handshake", [](uint8_t motor_count, uint8_t init_0, uint32_t baud) {
		...
		py::bytes result = py::reinterpret_steal<py::object>(PYBIND11_BYTES_FROM_STRING_AND_SIZE((char*)data, sizeof(CTRL_Handshake)));
		delete[] data;
		return result;
	});
	handle.def("get_CTRL_Handshake_data", [](char* handshake) {
		...
		uint32_t	baud;
		uint16_t	crc;
		get_CTRL_Handshake_data(*((uint64_t*)handshake), &motor_count, &init_0, &baud, &crc);
		return std::make_tuple(motor_count, init_0, baud, crc, sizeof(CTRL_Handshake));
	});
