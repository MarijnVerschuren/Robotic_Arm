...
uint8_t* new_MCU_Instruction(uint16_t id, uint8_t action, double target, double max_vel, double max_acc, uint8_t micro_step, uint8_t srd_mode, uint16_t* instrution_id) {
	MCU_Instruction* data =	(MCU_Instruction*)malloc(sizeof(MCU_Instruction));
	data->target =			target;
	...
	*instrution_id =		instrution_id_counter;
	...
	data->crc = 			crc16_dnp(data, sizeof(MCU_Instruction) - 2);
	return (uint8_t*)data;
}
...
uint8_t* new_CTRL_Handshake(uint8_t motor_count, uint8_t init_0, uint32_t baud) {
	CTRL_Handshake* data =	(CTRL_Handshake*)malloc(sizeof(CTRL_Handshake));
	data->motor_count =		motor_count;
	data->init_0 =			init_0;
	data->baud =			baud;
	data->crc = 			crc16_dnp(data, sizeof(CTRL_Handshake) - 2);
	return (uint8_t*)data;
}
