...
HAL_StatusTypeDef AS5600_init(AS5600_TypeDef* handle) {
	...
	if (!(handle->outputMode)) { handle->outputMode = AS5600_OUTPUT_STAGE_DEFAULT; }
	...
	if (AS5600_set_output_mode(handle, handle->outputMode, handle->PWM_frequency) != HAL_OK)	{ return HAL_ERROR; }
